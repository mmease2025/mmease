...
Installed from ***CRAN*** (can also from other repositories):
dummies,e1071,adabag,C50,pROC,kknn,MASS,multiROC,caret,mlbench,randomForest,varSelRF.

Installation commands:

``` r
CRAN_packages <- c("dummies","e1071","adabag","C50","pROC","kknn","MASS","multiROC","caret","mlbench","randomForest","varSelRF")
...
devtools::install_github("mmease/mmease")
```
...
``` r
#functional analysis
Table <- functional_differential(corrected_data,method = "t_test")
Table <- functional_differential(corrected_data,method = "ANOVA")
Table <- functional_differential(corrected_data,method = "FC")
Table <- functional_differential(corrected_data,method = "PLS-DA")
Table <- functional_differential(corrected_data,method = "OPLS-DA")
Table <- functional_differential(corrected_data,method = "RF_RFE")
Table <- functional_differential(corrected_data,method = "Kruskal_Wallis")
Table <- functional_differential(corrected_data,method = "svmrfeFeatureRanking")

#metabolic analysis
Table <- metabolic_differential(corrected_data,method = "t_test")
Table <- metabolic_differential(corrected_data,method = "ANOVA")
Table <- metabolic_differential(corrected_data,method = "FC")
Table <- metabolic_differential(corrected_data,method = "PLS-DA")
Table <- metabolic_differential(corrected_data,method = "OPLS-DA")
Table <- metabolic_differential(corrected_data,method = "RF_RFE")
Table <- metabolic_differential(corrected_data,method = "Kruskal_Wallis")
Table <- metabolic_differential(corrected_data,method = "svmrfeFeatureRanking")
```

Classification model

``` r
class <- classification(corrected_data,method = "AdaBoost")
class <- classification(corrected_data,method = "Bagging")
class <- classification(corrected_data,method = "Decision Trees")
class <- classification(corrected_data,method = "K-Nearest Neighbor")
class <- classification(corrected_data,method = "Linear Discriminat Analysis")
class <- classification(corrected_data,method = "Naive Bayes")
class <- classification(corrected_data,method = "Partial Least Squares")
class <- classification(corrected_data,method = "Random Forest")
class <- classification(corrected_data,method = "Support Vector Machine")
